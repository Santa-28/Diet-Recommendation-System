...
from ImageFinder.ImageFinder import get_exercise_images_links, get_images_links as find_image
...
class Person:
    ...
    def __init__(self, age, height, weight, gender,disease, activity, meals_calories_perc, weight_loss):
        self.age = age
        self.height = height
        self.weight = weight
        self.gender = gender
    ...
    def generate_recommendations(self):
        ...
        for meal in self.meals_calories_perc:
            ...
            response_data = generator.generate()  # Ensure generate() returns the response data
            if response_data:
...
class Display:
    def __init__(self):
        self.plans = ["Maintain weight","Weight Gain" ,"Mild weight loss", "Weight loss", "Extreme weight loss"]
                ...
                with column:
                    ...
                    for recipe in recommendation:
                        ...
                        expander.markdown(f"""
                            ...
                            """)
    def display_exercise_images(self, disease):
        """Displays exercise images based on the disease condition."""
        st.header('Recommended Exercises')
        exercise_images = get_exercise_images_links(disease)
        for exercise in exercise_images:
            st.image(exercise['link'], caption=exercise['description'], use_column_width=True)
    ...
    def display_meal_choices(self, person, recommendations):
        ...
        if len(recommendations) == 3:
            ...
            with afternoon_snack:
                afternoon_snack = st.selectbox(f'Choose your afternoon snack:', [recipe['Name'] for recipe in recommendations[3]])
            with dinner_column:
        ...
        total_calories_chosen = total_nutrition_values['Calories']
        loss_calories_chosen = round(person.calories_calculator() * person.weight_loss)
        ...
        total_calories_graph_options = {
            ...
            "series": [
                {
                    "data": [
                        {"value": total_calories_chosen, "itemStyle": {"color": ["#33FF8D", "#FF3333"][total_calories_chosen > loss_calories_chosen]}},
                        {"value": loss_calories_chosen, "itemStyle": {"color": "#3339FF"}},
                    ],
                    "type": "bar",
                }
            ],
...
with st.form("recommendation_form"):
    ...
    disease = st.selectbox('Select any disease:', ['None', 'diabetic','pcos','pressure','ulcer','Overall','other'])
...
if generated:
    ...
    person = Person(age,height,weight,gender,disease, activity,meals_calories_perc,weight_loss)
    with st.container():
...
if st.session_state.generated:
    with st.container():
        ...
        display.display_exercise_images(st.session_state.person.disease)
...
# from Generate_Recommendations import Generator
# from random import uniform as rnd
# from ImageFinder.ImageFinder import Not_found_link, get_exercise_images_links, get_images_links as find_image
# from streamlit_echarts import st_echarts
# # import json
# # import requests
...
#     def __init__(self, age, height, weight,disease, gender,activity, meals_calories_perc, weight_loss):
#         self.age = age
#         self.height = height
#         self.weight = weight
#         self.gender = gender
...
#     with st.container():
#         display.display_recommendation(st.session_state.person, st.session_state.recommendations)
#         display.display_exercise_images(st.session_state.person.disease)  # Correct call
#         st.success('Recommendation Generated Successfully !', icon="✅")
#     with st.container():
#         display.display_meal_choices(st.session_state.person, st.session_state.recommendations)
